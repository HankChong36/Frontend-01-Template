...
function calcMainAxisProperty(
  flexLines,
  style,
  { mainSize, mainBase, mainStart, mainEnd, mainSign }
) {
  let mainSpace = 0;
...
function calcCrossAxisProperty(
  flexLines,
  style,
  crossSpace,
  { crossSize, crossBase, crossEnd, crossStart, crossSign }
) {
  let start = crossBase;
  let step = 0;
  ...
  for (const key in style) {
    if (style.hasOwnProperty(key)) {
      const camelCaseKey = key
        .split("-")
        .map((str, idx) => (idx === 0 ? str : str[0].toUpperCase() + str.substr(1)))
        .join("");
