...
const { getBodyString, getSearchString } = require("./utils");

module.exports = class Request {
  constructor(options) {
    const { method, host, port, path, headers } = options;
    this.method = method || "GET";
    this.host = host || "127.0.0.1";
    this.port = port || 80;
    this.path = path || "/";
    this.base = `http://${this.host}:${this.port}${
      this.path.indexOf("/") === 0 ? this.path : "/" + this.path
    }`;
    this.headers = headers || {};
    this.socket = null;
  }

  send({ path, conf, callback }) {
    conf = conf || {};
    let { method, headers, data = {}, body = {} } = conf;

    method = method || this.method;

    headers = Object.assign(
      {
        "Content-Type": method === "GET" ? "text/plain" : "application/x-www-form-urlencoded"
      },
      this.headers,
      headers
    );
